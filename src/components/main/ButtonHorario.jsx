...
export default function ButtonHorario({ horarioss }) {
  const [selectedHora, setSelectedHora] = useState("");
      ...
      <div className="flex flex-wrap gap-2 max-w-xl mt-5 justify-center">
        {horarioss.map((hora, index) => (
          <label
            className={`${
              selectedHora === hora.hora ? "outline outline-4 outline-teal-700" : ""
            } text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 cursor-pointer w-20 select-none`}
            key={index}
          >
            <input
              className="appearance-none"
